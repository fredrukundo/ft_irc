...
/*   Updated: 2024/10/04 10:29:49 by frukundo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::getCmdArgs(std::string cmd, std::string &name, std::string &modeset, std::string &params)
{
	...
	size_t pos = cmd.find(modeset) + modeset.length();

	pos = cmd.find_first_not_of(" \t\v", pos);
...
void Server::mode_command(std::string &cmd, int fd)
{
	...
	if ((channelName[0] != '#' && channelName[0] != '&') || !(channel = GetChannel(channelName)))
	{
	...
	if (chain.empty())
		return;
	channel->sendTo_all(RPL_CHANGEMODE(cli->get_client_host(cli->getIpAdd()), channel->GetName(), mode_chain.str(), arguments));
}
...
std::string Server::invite_only(Channel *channel, char opera, std::string chain)
{
    ...
    {
		senderror(471, channel->GetName(), cli->GetFd(), " :Cannot join channel (+l)\r\n");
        return "";
    }
...
std::string Server::channel_limit(std::vector<std::string> tokens, Channel *channel, size_t &pos, char opera, int fd, std::string chain, std::string &arguments)
{
	...
	if (opera == '+')
	{
		if (tokens.size() > pos)
		{
			...
			else if (channel->GetClientsNumber() >= std::atoi(limit.c_str()))
			{
				_sendResponse(ERROR_CHANNELISFULL(cli->GetNickName(), channel->GetName()), fd);
			}
			else
			{
